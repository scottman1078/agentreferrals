...
export default function AgentHoverCard({ agent, position }: AgentHoverCardProps) {
  ...
  return (
    <div style={style}>
      <div className="w-[240px] bg-card/95 backdrop-blur-xl rounded-xl shadow-2xl border border-border p-3 animate-in fade-in zoom-in-95 duration-150 relative overflow-hidden">
        {scopeLocked && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-card/50 rounded-xl">
            <button onClick={() => window.location.href = '/dashboard/settings?tab=billing'} className="text-xs font-bold text-primary-foreground px-4 py-2 rounded-lg bg-primary shadow-lg hover:opacity-90 transition-opacity" style={{ pointerEvents: 'auto' }}>Upgrade to view</button>
          </div>
        )}
        <div className={scopeLocked ? 'blur-[3px]' : ''}>
